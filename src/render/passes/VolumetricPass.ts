...
export function createVolumetric(
  ...
  const node = (() => {
    ...
    const jitter = hash(
      screenCoordinate.x.mul(1973).add(screenCoordinate.y.mul(9277)).add(time.mul(60).floor().mul(26699)),
    );
    ...
    Loop({ start: 0, end: steps, type: 'int', condition: '<' }, ({ i }) => {
      ...
      const attenuation = falloff.mul(falloff).div(torchDistance.mul(torchDistance).mul(0.35).add(1));
