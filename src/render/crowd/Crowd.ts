...
export class Crowd {
  ...
  sync(dt: number, cameraPosition: Vector3, torch: Torch | null): void {
    ...
    for (const slot of this.slots) {
      ...
      if (slot.lod === 2) {
        yaw = Math.atan2(
          cameraPosition.x - slot.position.x,
          cameraPosition.z - slot.position.z,
        );
      }
      ...
      const flags =
        (slot.hitFlash > 0 ? 1 : 0) + (inCone ? 2 : 0) + (slot.holdLastFrame ? 4 : 0);
      ...
      if (bucket.tile) {
        ...
        const yawTile = Math.round(((relative / (Math.PI * 2)) * BILLBOARD_YAWS + BILLBOARD_YAWS) % BILLBOARD_YAWS);
