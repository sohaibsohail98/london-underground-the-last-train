...
function openness(context: EmitterContext, cells: { x: number; y: number }[], dx: number, dy: number): number {
  let count = 0;
...
export function emitPlatform(context: EmitterContext): void {
  ...
  for (const run of runs) {
    ...
    const trackDirection = openness(context, run.cells, ...first) <= openness(context, run.cells, ...second)
      ? first
      : second;
    ...
    lip.addBox(centre.clone().add(edgeOffset).setY(PLATFORM_LIP / 2), runSize, 0.6, 0.7);
    ...
    const warningMatrix = new Matrix4().compose(
      centre.clone().add(warningOffset).setY(PLATFORM_LIP + 0.012),
      identity,
      alongX ? new Vector3(length / 2.6, 0.06, 0.42 / 0.28) : new Vector3(0.42 / 2.6, 0.06, length / 0.28),
    );
    context.strips.push({
      kind: 'warning',
    ...
    bed.addBox(
      bedCentre,
      alongX
        ? new Vector3(length, 0.4, TRACKBED_WIDTH)
        : new Vector3(TRACKBED_WIDTH, 0.4, length),
      0.75,
      0.6,
    );
    ...
    wall.addBox(
      backCentre,
      alongX ? new Vector3(length, 3.6 + TRACK_DROP, 0.4) : new Vector3(0.4, 3.6 + TRACK_DROP, length),
      0,
      0.42,
    );
    ...
    const stripMatrix = new Matrix4().compose(
      centre.clone().add(edgeOffset).setY(PLATFORM_LIP + 0.08),
      identity,
