...
function drawPanel(
  ...
  gradient.addColorStop(1, `#${base.clone().offsetHSL(shift * -1, 0, -0.42).getHexString()}`);
...
export function emitSignage(context: EmitterContext): void {
  ...
  for (const cell of everyNth(adCells, 6, 2)) {
    ...
    face.position.copy(position).addScaledVector(new Vector3(Math.sin(facing), 0, Math.cos(facing)), 0.05);
  ...
  for (const cell of everyNth(entrances, 7)) {
    ...
    context.lightCandidates.push({
      position: position.clone().addScaledVector(new Vector3(Math.sin(facing), 0, Math.cos(facing)), 0.5),
