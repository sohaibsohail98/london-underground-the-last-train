...
| `phase-f1a-pcg-dressing.md` | Authoring-time PCG surface dressing: cable runs, wall and floor grime and clutter, baked to static instances. Spec only, no `Source/` change, does not touch layout or gameplay placement | CC-in-Unreal | nothing, do alongside or after F1 |
| `phase-f8-menu-polish.md` | Dress the main menu scene (S13) with F1 kit pieces and the F2 look, menu ambience, optional slow camera drift | CC-in-Unreal | F1, F2 |
