...
| File | Task | Who | State |
|---|---|---|---|
| `phase-b1-throttled-repath.md` | Throttle per zombie `MoveToActor` to a jittered cadence | Opus | C++ landed. RVO radius 90 to 45, acceptance `0.75f` to `0.5f`, then a `DriveTowardsTarget` rework with `ContactRange` and a stall-recovery nudge, all on top (supersedes the "do not change the movement setup" line in the spec). The 24 to 40 crowd frame check is unmeasured but unblocked: the placed round manager accepts an `OpeningRoundCounts` write since `bb0ec42`. The corridor stall edge case has a verdict as of 2026-09-07: the repath in `Tick` was re-issuing the AI move over the recovery nudge, and the stall timer could not accumulate through jitter. Fixed, not compiled, not seen in PIE. See `phase-c-review-2026-09-07.md`. |
...
| `phase-c-special-rounds.md` | Sprinter round every 5th, brute pair every 10th, as a plan layer on `ALTRoundManager` | Opus | **C++ written** on top of the roster: `FLTRoundPlan`, `BuildRoundPlan`, the three-way branch in `TrySpawnOne`, `IsSpecialRound` and `GetSpecialRoundTag`. 4 CI gates pass. **Not compiled.** Reviewed against its spec on 2026-09-07 (`phase-c-review-2026-09-07.md`): the brute pair lands at roughly 30 and 70 per cent through the round rather than as a group up front, which canon asks for and which stands. The second deviation turned every brute round into a sprinter round, round 10 included, and is now the `bBruteRoundOverridesSprinterRound` tick, default set, so a brute round is a walker round plus the pair. Three bugs fixed in the same pass: a brute could lose its spawn slot on a short round, the banner outlived a stopped run, and the spawn log printed the base cap rather than the effective one. |
