...
| S14 Phase F asset research + G2 HUD spec (remote session) | **DONE 2026-09-09. Docs and one script only, no `Source/` change, no `.uasset`, nothing downloaded.** <br> **Asset research.** Four research subagents swept the CC0 world for Phase F. Output is `docs/reference/asset-sources-phase-f.md`: surfaces (F1), meshes (F1/S13), architectural reference photography (F1), train shape reference (F3), menu ambient audio (S13), plus an "inspect before staging" list and a plain statement of what the research could not settle. **The download half of the spec was impossible and the doc says so up front:** the remote container's egress proxy refuses every asset host (ambientcg, polyhaven, kenney, quaternius, freesound, itch.io, wikimedia all 403 at CONNECT; only GitHub, the Anthropic API and the package registries are reachable), and separately `_incoming_assets/` is gitignored and dies with the container, so a staged file has no route to the Mac. Web search still works, so the research itself is real. `tools/asset-fetch/fetch-phase-f.sh` closes the gap: run it on the Mac and it creates `_incoming_assets/<category>/` with a `SOURCES.txt` per category, pulls the 36 ambientCG and 22 Poly Haven sets through their keyless APIs, and prints a manual checklist for everything needing a human click. It imports nothing and runs no git. Tested here for syntax, dry run, and graceful failure against the blocked proxy; **not tested against a live host, because this container cannot reach one.** <br> **Headline findings.** (1) **There is no CC0 modular station kit.** Nothing on Quaternius, Kenney, Poly Haven, ambientCG, OpenGameArt or the CC0 corners of itch.io ships wall panels, ceiling sections, platform-edge sections, escalators and tunnel portals as one public-domain set; every realistic one found is paid. The closest free thing (loafbrr Modular Underground Metro) is CC-BY 1.0, not CC0, and is a cramped metro bore. **F1 must budget for modelling the shell, platform edge, escalator, gate line and tunnel portal in house.** (2) Surfacing is well covered: hero tile pair is ambientCG `Tiles036` clean + `Tiles133B` dirty, same white family so they blend without a hue shift. (3) For F3, **the Class 345 dimension table is complete enough to box-model from and no CC0 kitbash mesh is worth using**; the standing recommendation is to work from Class 720/701 reference (same body section and cab family, non-TfL operators, no purple) plus the 345 numbers for the three-door spacing, so the livery cannot bleed in by osmosis. The best single reference find is three FOI requests on WhatDoTheyKnow answered with dimensioned top/side/front drawings, though their re-use notice is unread. (4) Menu audio should be a **synthesised** CC0 drone plus a room tone, not a field recording, because a synthesised drone provably cannot contain an announcement. <br> **Four open legal flags, listed in section 6 of the manifest:** the signboards and posters in Modular Underground Metro, the bus stop in City Environment Pack #2, photogrammetry scans that may carry painted markings (ambientCG `Road001` named specifically), and the unread FOI re-use notice. **Every licence in the document rests on search evidence only: no page was opened and no file was looked at.** <br> **G2 HUD spec.** `docs/tasks/phase-g2-hud.md`. Binding table (every value the HUD needs is already Blueprint-visible, so G2 needs no `Source/` change and no compile), layout, per-element behaviour, the two states B3 never had (downed overlay off the bleed-out clock, run-over card, which also closes the S13 follow-up that a death had no route back to the menu), all 5 open S7 findings, and the cut list. Notes the one indirection: `OnDamageTaken` / `OnDowned` / `OnRevived` / `OnBleedOutExpired` / `OnDied` are protected `BlueprintImplementableEvent` on `ALTPlayerCharacter`, not assignable delegates, so the player Blueprint implements and forwards them. Flags two owner decisions: the typeface split between menu and HUD, and whether the station schematic belongs in the HUD at all (recommends not, while v1 ships two stations: it belongs in F5 as diegetic platform signage). Also notes `docs/art-direction.md` section 7 is now stale, since it says Overpass is not in the project and the OFL faces are in fact imported. <br> All five CI gates pass. |
| F1 Modular kit + Canary Wharf shell | **DONE 2026-09-09. Editor only, no `Source/` change.** Commits `a0fcdba` (kit assets) and `47f4bac` (map). **New: 12 original kit meshes** in `Content/LastTrain/Kit/Meshes/`, authored in engine with NeoStack geometry scripting (box and cylinder primitives plus booleans): wall panel and a service-recess variant, floor panel and a tactile-strip variant, coffered ceiling panel, pillar, platform edge, arched tunnel portal, stair run, bench, bin, handrail. Each is one closed mesh, Nanite on and built, with a real material slot. Module contract and the legal position are in `Kit/KIT-NOTES.txt`. **New surfaces:** the hero tile pair the S14 research names, ambientCG `Tiles036` clean and `Tiles133B` dirty, both CC0. Fetching them also settles the first unknown in known-issues 2.1: **the ambientCG zip URL shape `https://ambientcg.com/get?file=<ID>_2K-JPG.zip` that `fetch-phase-f.sh` uses does resolve against the live host** (HTTP 200, ~14 MB each). **Map:** 201 scaled `/Engine/BasicShapes/Cube` structural actors replaced; 701 actors now carry a kit mesh and the only engine cubes left are the 8 gameplay anchors and the 22 F5 signage pieces. 405 wall panels, 74 platform-edge pieces, an 8-bay pillar colonnade, 60 floor and 60 ceiling panels, 2 tunnel portals, plus escalators, barriers, benches, bins, debris doors and boarding repointed. Actor count 374 -> 849, lights unchanged at 55. **The wall module is 150 uu, not 400:** the blockout is built on a 150 grid and 400 divides none of its wall runs, so the 400 panel is scaled to the grid rather than the map being pulled off it. Geometry was replaced **in place** by repointing each existing actor, so no gameplay actor was ever deleted and recreated. **Three real defects found and fixed, worth knowing before the next kit task:** (1) **`geometry_uvs(mode="recompute")` silently destroys usable UVs.** It returns success and leaves `uv_channels` at 1, but every surface renders flat and untextured. Proved with an A/B test: a raw `geometry_create` box and an engine cube both textured correctly under the same material instance while the recomputed kit panel did not. `geometry_create` already emits good UVs. All 12 meshes were rebuilt without that call. (2) **Geometry-scripted meshes ship with zero collision shapes**, so they are invisible to navmesh generation. Before the fix **no spawn point was on the navmesh at all** and the station was completely unwalkable. Every piece now has simple box collision, except the tunnel portal, stair run and platform edge, which use `CTF_UseComplexAsSimple` so the arch stays open, the treads stay climbable and the fall to track level is real. (3) The portals were first placed on the tunnel-bay centres (4050 / 7050) rather than the measured centres of the 300 uu gaps in the `y=6225` wall row (4125 / 7125), which put a jamb inside the horde's doorway. Both portals and both tunnel bays were moved onto the true gap centres. **Also enabled `r.Shadow.Virtual.Enable`** in `Config/DefaultEngine.ini`, which Nanite asks for and F2 wants anyway. That plus the material slots took Map Check from **1100 warnings to 0**. **Verified:** 34 gameplay actors read back at their exact pre-F1 transforms (player start, round manager, train, both nav volumes, departure board and its two panels, all 10 spawn points, all 8 anchors, the 4 lighting actors, 4 signage pieces); Map Check **1 error / 0 warnings**, the error being the stock "Maps need lighting rebuilt" every map here carries under Lumen (`BUILDLIGHTING` is not even a recognised command in this fully dynamic project); **all 10 spawn points return a complete, non-partial navmesh path to the player start** (lengths 2676 to 9452); and a PIE round shows a zombie crossing the platform and reaching the player with the tiled hall, pillars, coffered ceiling and platform edge all reading correctly. PIE stopped cleanly both times; nothing was mutated while it ran. **Not done:** `L_GreyboxTest` still has its Phase B cube geometry, so F1 is complete for Canary Wharf only. Lighting is unbalanced against the new near-white tile (badly blown out in places), which is F2's job, not a kit defect. The train is still two scaled panels standing in for F3. |
| F2 Lighting, Lumen and atmosphere | **DONE 2026-09-09. Editor only, no `Source/` change.** **The F1 blowout is fixed, and it had four causes, all confirmed by property read rather than guessed:** (1) **every light in the map had `CastShadows=False`**, so light accumulated unopposed and nothing anywhere cast a shadow; (2) `AutoExposureApplyPhysicalCameraExposure=True` was layering a physical camera stop (ISO 100 / shutter 60) on top of an exposure already locked at min=max=1.0; (3) `SceneColorTint` was `(0.95, 0.88, 0.82)`, a global sepia wash, which is why the whole frame read warm including the ceiling rather than only the sodium pools; (4) the platform floor was `MI_Concrete046` at roughness 0.60 to 0.95, bone dry, so there was no reflection to break the light up. **Post process** (`CW_D_PostProcess`, already unbound): exposure method set to `AEM_Manual` with the physical-camera stop **off** and bias `-1.35`; `SceneColorTint` returned to neutral white so warmth comes from the lights; vignette `0.50 -> 0.22`. Lumen GI and reflections were already on and hardware ray tracing already off, so neither needed changing. **Lighting rig:** the 32 pack fittings (`BP_Overheadlight_01_C`) are from the gitignored `/Game/SubwayTrain/` pack and their light components are `Mobility=Static` on a Blueprint template, so **their intensity and shadow flags cannot be set from the level** and `SetMobility` is refused on the instance. Rather than edit a pack Blueprint, **32 new movable shadow-casting sodium spot lights** (`CW_F2_Sodium_00..31`) were added one under each fitting at `z=322`, 2000 cd, `#E0A030`-family colour, 18/48 degree cone. The pack's own bulb light was dimmed to 260 cd / 150 uu radius so it still reads as a lit filament without throwing the room. The 18 flat fill lights (`CW_D_Fill_*`) went `700 cd -> 90 cd` and were cooled to a neutral blue-grey, so they lift the shadows without flattening them. **Sky light** moved off flat grey onto the project's existing `HDRI_dresden_station_night_4k` cubemap (`SLS_SpecifiedCubemap`, intensity 0.9) for a real underground ambient; the directional sun was cut to 0.02 and cooled, since a station is underground. **Emergency lighting:** 8 crimson `#B02030` point lights (`CW_F2_Emerg_T1_1..4`, `T2_1..4`) receding down both tunnel mouths on a **rising** intensity gradient (130 / 210 / 320 / 430 cd) so the eye is drawn into the tunnel. `volumetric_scattering_intensity=5.0` so they tint the haze. They are not general illumination and the rest of the map is not red. **Fog:** `CW_HeightFog` retuned (density `0.080 -> 0.035`, falloff `0.35 -> 0.12`, start 300, max opacity 0.85) and **volumetric fog enabled** (`bEnableVolumetricFog`, extinction 1.4, distance 9000, start 120). **Wet floor, which carries most of the look:** new `MI_LT_FloorWet` in `Materials/Dressing/`, parented to the existing `M_LT_PBRSurface` master, carrying the Concrete046 textures with roughness remapped to `0.08..0.34` and tinted down to `(0.26, 0.255, 0.28)`. Applied to all 60 `CW_Floor_*` panels. Lumen reflections then do the rest: sodium smears down the platform exactly as `art-direction.md` predicts. **Surfaces rebalanced against the new rig:** `MI_Concrete036` (the 60 ceiling panels, and nothing else uses it) darkened to `(0.20, 0.195, 0.21)` at roughness `0.72..0.96` so the soffit stops acting as a giant bounce card; `MI_Tiles036`, which turns out to be on **the 8 pillars only**, not the walls, brought down off near-white to `(0.52, 0.51, 0.50)`; `MI_Tiles133B`, the 405 wall panels, to `(0.62, 0.60, 0.58)`. **Verified:** Map Check **1 error / 0 warnings**, the error being the stock "Maps need lighting rebuilt"; **all 30 named gameplay actors byte-identical to their pre-F2 transforms**, checked by hashing the sorted transform dump before and after (`787847072` both times); **all 10 spawn points return complete, non-partial navmesh paths** to the player start at the same lengths F1 recorded (2676 to 9452); a PIE round ran with zombies crossing the platform and reaching the player, the HUD reading Round 1 / 500 points / Stag Compact 30|90, and the lighting reading correctly through the real player camera. PIE was stopped cleanly both times and nothing was mutated while it ran. Actor count `849 -> 889` (+32 sodium, +8 emergency), lights `55 -> 95`. Acceptance frames are `docs/reference/f2-acceptance-platform.png` and `f2-acceptance-tunnel.png`. **Three things the next session needs to know.** (1) **No frame-rate baseline was captured, and the number is not guessed.** In PIE over the MCP bridge the reported world delta pins to a dead-flat `0.33333 s` (3 fps) and neither `t.MaxFPS 0` nor `r.Editor.ThrottleCPUWhenNotForeground 0` shifts it, while game real-time still advances 8.001 s over an 8.0 s wall-clock wait, so the **game clock is running at true speed and the smoothed delta is what is capped**, not the renderer. `stat unit` does not render into either screenshot path. F7 must measure this with the editor focused. (2) **A broken material, `M_LT_PlatformWet`, is on disk and deliberately NOT committed.** It was a first attempt at a puddle-masked wet floor with a world-position-driven noise mask. A `recompile_material` call **timed out at the NeoStack 60 s limit mid-build** and left the asset compiling to **0 pixel-shader instructions** (the working `M_LT_PBRSurface` compiles to 379), so it rendered as an untextured default and a deliberately near-black test tint produced no visible change at all, which is what identified it. `delete_asset` refuses it because it is held in memory, exactly the trap F5 hit with `M_LT_SignTextUnlit`. It is unreferenced, superseded by `MI_LT_FloorWet`, and left untracked; delete it from disk after an editor restart. **Check `num_pixel_shader_instructions` via `MaterialEditingLibrary.get_statistics()` after authoring any material this way: a graph can be fully wired, save, and still not compile.** (3) **F5's sign dimming is not fixed and was deliberately left alone.** The West sign at CW `X=3300` is legible in the new rig and reflects well in the wet floor, but it sits in a dark pocket between fittings. Three dedicated sign wash lights were tried and **reverted**: they did not land on the sign face and the volumetric fog caught the beams as visible cones. The root cause stands as F5 diagnosed it, `TextRenderComponent` using the engine's lit `DefaultTextMaterialOpaque`, and still wants a purpose-built unlit text material rather than more lights. **Not done:** the reference frame's signature composition, the train as a full-height wall down one side, is **not reachable yet**: `CW_Train` sits at `y~5295`, out at track level, still the F3 two-panel stand-in, so the platform corridor was framed instead. `L_GreyboxTest` is untouched by F2 and still carries the Phase B lighting. |
| F3 Train exterior mesh and livery | **PARTIAL 2026-09-09. Editor only, no `Source/` change.** The train exterior is built, placed and reading; **the door animation is wired but deliberately left inert, and the frame does not yet stand next to the reference.** Both are stated plainly below. **First action, per known-issues 2.7:** deleted the dead `M_LT_PlatformWet.uasset` from disk on a fresh editor start; working tree was clean afterwards apart from F3's own changes. **6 new meshes** in `Content/LastTrain/Train/Meshes/`, authored with geometry scripting: `SM_Train_BodySection`, `SM_Train_DoorBay`, `SM_Train_CabFront`, `SM_Train_DoorLeaf`, `SM_Train_Skirt`, `SM_Train_RoofPod`. All Nanite, all with box collision and a real material slot. Built to the `rolling-stock.md` numbers: body width 277 uu (2.772 m), rail to roof 376 uu (3.760 m), rounded-rectangle section with 34 uu cant-rail and solebar fillets, doorway 145 uu leaf pair, 200 uu tall. The cab is a plan-curved wraparound nose: a cylinder intersect pulls the front face back 55 uu at the corners (x=105 at centre, x=50 at y=+/-138.5), plus a 9 degree roof rake, a recessed windscreen band and headlight and coupler recesses. **Layout:** 4 cars, 12 body sections of 750 uu plus two 210 uu noses = 9420 uu, spanning x 840..10260 against a 10950 uu platform, actor z **-36** so the door sill sits 12 uu above the platform slab, the roof at z=340 with 75 uu of ceiling clearance, and the skirt buried 36 uu into the slab so it reads as a shadow gap. **125 new actors** under `TrainShell/F3*`: 2 cabs, 12 body sections, 14 skirts, 24 door leaves, 36 violet door surrounds, 2 sodium cab bands, 6 original operator marks, 6 roof pods, 1 lit interior backdrop, 12 cabin rect lights, 10 exterior sodium wash spots. **Livery is original**, palette only: charcoal bodyshell, `#6C4C9C` violet door surrounds, `#E0A030` sodium cab band, a made-up three-bar operator mark, no wordmark, no roundel, no Johnston, nothing from `_incoming_assets/props/underground-diorama/` was used at all so its roundel texture cannot leak. **The two old stand-in panels `CW_TrainCarriage_1/2` were hidden, not moved or deleted**, so the 30-actor transform set is untouched. <br> **Three findings worth carrying forward.** (1) **`geometry_create` boxes are centred in x and y but sit ON z=0**, base at the origin, and `geometry_transform` translate is **relative and cumulative**. Getting this wrong silently puts geometry 168 uu high. (2) **A recessed window band shows nothing behind it.** The first body mesh recessed the glazing 10 uu into the skin rather than cutting through, so the lit interior was invisible; both body meshes were rebuilt with the band cut clean through, and the collision then has to be **two side-wall boxes, not one body box**, or the aperture is sealed to the player. Removing the leftover full-width box is a separate `remove("collision",{index=0})` after the rebuild. (3) **A geometry rebuild drops the material slot**, and `configure("material", 0, ...)` fails with "index out of range" on a mesh that has none; the slot has to be created through `static_materials` in Python first. <br> **Materials.** 9 kept, all verified compiling. The eight lit ones are instances of the existing, proven `M_LT_PBRSurface` and all report **379 pixel-shader instructions**, so no new master was needed for them and known-issues 2.7 could not bite. One new master **was** needed for the lit cabin, `M_LT_TrainEmissive` (unlit, `Colour` and `Brightness` parameters). It was authored **exactly to the 2.7 procedure: built and saved in one call, compiled in a separate call, then verified**, and reports **96 pixel-shader instructions**, non-zero. Its four instances also report 96. `MI_Train_Glass` and `MI_Train_Interior` were superseded, confirmed to have zero referencers, and deleted. **The livery had to move well off the literal palette hex to read at all:** `#16161C` is linear 0.008 and rendered as a light-absorbing void, so the bodyshell tint sits at 0.105 with the palette's blue-violet cast kept. The Metal032 basecolour and normal were also dropped for flat white plus `DefaultNormal`, because that texture's fine grille pattern read as a mesh screen rather than a painted bodyshell. <br> **Verified.** The **30 gameplay actors are byte-identical to their pre-F3 transforms**: sorted transform dump hashed **3974687351 before and 3974687351 after** (this session's own djb2 fold, not F2's 787847072, which came from a different algorithm). **All 10 spawn points return complete, non-partial navmesh paths** to the player start at **2676.1 to 9452.4**, the same lengths F1 and F2 recorded. Map Check **1 error / 0 warnings**, the error being the stock "Maps need lighting rebuilt" every map here carries under Lumen. All 5 CI gates pass. All 19 new `.uasset` files are LFS-tracked (`git check-attr filter` returns `lfs`). Nothing was mutated while PIE was running, because **PIE was never started this session**. <br> **NOT DONE, and the next session should start here.** (1) **The doors do not move.** `BP_Train` now carries a `DoorSlide` timeline (1.4 s, `Alpha`/`OffsetL`/`OffsetR` tracks), `DoorLeavesL`/`DoorLeavesR`/`DoorClosedX`/`DoorOpenX` array variables, a BeginPlay chain that collects the 24 tagged leaves via `GetAllActorsWithTag` on `LTDoorL`/`LTDoorR`, `OnDoorsOpen` -> `PlayFromStart` and `OnDoorsClose` -> `Reverse`, and a full Update -> ForEach -> `Set Actor Location` chain with the vector pins split. It **compiles clean with 0 errors and 0 warnings.** What is missing is the per-leaf closed and open X. The `Lerp` node is placed and its `Alpha` is wired, but `DoorClosedX`/`DoorOpenX` are never populated, so an empty array would read 0 and teleport every leaf to x=0. Rather than ship that, **the X pin was deliberately re-wired to pass the leaf's current X straight through**, which is inert and safe: the doors sit correctly closed and nothing jumps. To finish it, populate the two arrays in the BeginPlay loop (needs a float add, which did not surface through `find_nodes` under `Add`, `Subtract` or `*` and wants a different lookup), then move the `Set Actor Location` X pin back onto the `Lerp` output. The 24 leaves carry `LTTrainDoor` plus `LTDoorL`/`LTDoorR` tags and are attached to `CW_Train`. (2) **No PIE round was run**, so the train cycle, boarding, zombie pathing and the HUD are **unverified this session**; the navmesh evidence above is static analysis, not play. (3) **The frame does not yet stand next to `docs/reference/reference-frame.png` and should not be claimed to.** The train reads as a dark wall with a lit window band receding correctly to the vanishing point, but the near-white F1 platform kerb still out-competes it for attention, and the violet surrounds and sodium cab band are not legible from the platform at the reference camera. The exterior wash rig (10 spots at 9000 cd) is a first pass and wants a proper balance against F2's existing rig. (4) The cab-end headlight, tail-light and door-button emissive instances are authored and verified but **not yet placed on any actor**. (5) `L_GreyboxTest` is untouched. |
| F3c Train doors, emissives and first PIE round | **DONE 2026-09-10. Editor and Blueprint only, no `Source/` change.** Closes the three gaps the F3 row left open. **The doors now move, and it was verified in PIE, not inferred.** <br> **The F3 blocker was two separate bugs, and the second one was the real one.** (1) The float add F3 could not find is the **promotable operator node in category `|Utilities|Operators`**. `find_nodes("Add")` returns it as result **1**, labelled `TimeManagementBlueprintLibrary` and titled "FrameNumber + Int", which is why F3 read past it. Spawn it, connect a `real` pin to `A`, and it **retitles itself `float + float`** with all three pins resolved to `real`. Searching for `Add_FloatFloat`, `Add_DoubleDouble`, `float + float` or `Adds two floating point` returns **0 results**, so the only route to it is the operators-category entry. (2) With the arrays correctly populated the doors **still did not move**, and this was the expensive one: **the 24 door-leaf `StaticMeshComponent`s were `Static` mobility**, and a Static component silently ignores `Set Actor Location` at runtime. Nothing errors, nothing warns, the Blueprint compiles clean and the timeline runs to completion. This is the same family as the **F2 Static-mobility ceiling-fitting note**. Setting all 24 to `Movable` fixed it outright. See known-issues **2.10**. <br> **How it was diagnosed, because the method is reusable.** Log search is unreliable here: `playtest_log_contains` returned `found=false` even for the string "PIE", so **the absence of `PrintString` output proved nothing**. The working technique was **temporary probe variables** read back through `playtest_read_state`: a `DoorHookFired` int set on `OnDoorsOpen` proved the C++ hook fires (`=1`), and a `LastAlpha` real set from the timeline's `Update` proved the timeline runs to completion (`=1.0` mid-dwell, `=0.0` when Away). That isolated the failure to the one link left: the `Set Actor Location` call itself. All three probe variables and both probe nodes were removed afterwards and the graph recompiled 0/0. <br> **What landed.** `BP_Train` gained `DoorClosedXR`/`DoorOpenXR` (array:real) so each side keeps its own parallel arrays. Two BeginPlay fill loops run after the existing tag collection: for each leaf, `Get Actor Location -> X` is appended to the closed array and `X + offset` to the open array, offset **-72** for the L side and **+72** for the R side. Filling both arrays in the same loop makes index correspondence true **by construction**, which is what makes `GetAllActorsWithTag`'s arbitrary order harmless. A second Update ForEach drives the R side off `DoorClosedXR`/`DoorOpenXR`, chained after the L loop's `Completed`. The F3 passthrough was undone: `Lerp.Return Value` is back on `Set Actor Location.New Location X` on both sides. The F3 timeline was replaced with `DoorSlide2` (1.4 s, single `A2` float track 0 to 1) while diagnosing; the old one and F3's orphan `MakeVector`/`BreakVector` are gone. Compiles **0 errors / 0 warnings**. <br> **32 emissive actors placed** under `TrainShell/F3Lamps`, all `NoCollision` and `cast_shadow=false`, so **no dynamic lights were added and the F2 budget is untouched**. 8 cab lamps: 2 white headlamps outboard at local y=+/-95 and 2 crimson tail lamps inboard at y=+/-55, on both cabs, sitting proud of the plan-curved nose at local x=82 and 86 (the nose surface at y=95 is x=81.3, from F3's circle centre -96.9 radius 201.9). 24 sodium door buttons, one each side of every doorway at x = centre +/-88, z=74, **y=5112 not 5119**: at 5119 they were **inside** the bodyshell skin at y=5123 and invisible, and this was caught by screenshot, not by any query. **`MI_Train_TailLight` Brightness was dropped 5.0 to 1.6**, because at 5.0 the emissive washed past the palette and rendered pink rather than `#B02030`; it re-verified at **96** pixel-shader instructions, non-zero, per known-issues 2.7. <br> **Verified in PIE across five sessions, all stopped cleanly with `playtest_stop`, no EndPlayMap crash, and nothing mutated while PIE was running.** **Train cycle:** `Phase` observed stepping Away -> Approaching -> Dwelling -> Departing -> Away on its own timer. **Doors:** mid-dwell every sampled leaf sits at its exact computed open X, `DoorL_01=1317` (closed 1389), `DoorR_01=1533` (closed 1461), `DoorL_06=5067`, `DoorR_06=5283`, `DoorL_12=9567`, `DoorR_12=9783`; while Away all read closed at 1389/1461. Re-confirmed on the production graph **after** the probes were stripped. **Zombies:** they spawned, pathed to the player and killed him, `RunState` reaching **`Downed`**; three are visible attacking in the PIE screenshot, so both pathing and the attack apply damage. **HUD:** live in the same screenshot, ROUND 1, 500 points, "Stag Compact" 30\|90. <br> **Acceptance.** 29 gameplay actor transforms hashed **3356498618 before and after** (this session's own djb2 fold over the `Gameplay`, `Spawns`, `Anchors`, `Interchange` and `TrainShell` folders plus `CW_Train` and `CW_DepartureBoard`; F3 quoted 3974687351 for a set it described as 30, and the two are **not comparable**, so this row states its own set explicitly). All 10 spawn points return **complete, non-partial** paths at **5335.6 / 3666.9 / 2676.1 / 3587.5 / 5203.2 / 6844.2 / 9025.0 / 9452.4 / 7264.4 / 5974.9**, identical to F1/F2/F3, so making the leaves Movable did not disturb navigation. Map Check **1 error / 0 warnings** via `lvl:map_check()`, the stock lighting-rebuild message; the banned console exec was not used. All 5 CI gates pass. 3 files touched, all LFS pointers. <br> **NOT DONE.** (1) **Boarding was never exercised.** The player was killed by zombies before a dwell could be used to board, so the C1 interact and the C3 `OpenLevel` travel path were left unverified. **Now exercised in F3d below, and boarding is broken: see known-issues 2.11.** (2) **The reference-frame lighting balance is untouched**, deliberately, per the F3c brief: it stays a Fable-lane job and the train still reads as a dark wall against F1's near-white kerb. (3) The doors-open screenshot is an editor-viewport capture; PIE's own camera is at the player start facing away from the train and the zombie crowd fills the frame, so no in-PIE doors-open frame was captured. (4) `L_GreyboxTest` is still untouched by Phase F. |
| F3d Boarding verification (C1 interact, C3 travel) | **VERIFIED FAILURE 2026-09-10. No change made: nothing edited, nothing saved, no commit needed for the finding itself.** This session existed only to close the one gap F3c left: boarding had never been exercised. It has now been exercised, and **boarding does not work in `L_CanaryWharf_Greybox`.** The C1 interact never becomes available, so the C3 `OpenLevel` travel path was never reached and remains unverified. Full detail in known-issues **2.11**. <br> **The evidence.** The player was walked from `CW_PlayerStart` to (5699, 4909, 132) on the platform, facing the train, god mode on so a zombie could not end the test. `CurrentInteractable` on `LTInteractionComponent` was sampled every 1.2 s across a complete dwell, from the first frame of `Dwelling` through to `Departing`, 22.0 s of samples: **empty on every one**, well past the 1 s `DoorOpenDelaySeconds`. A `player_view` capture with the crosshair centred on the train flank shows **no prompt**. <br> **The cause, measured rather than guessed.** `ULTInteractionComponent` sweeps a 12 uu sphere on `ECC_Visibility` out to `InteractionRange` 250, takes the **single blocking hit**, and only then tests for `ULTInteractableInterface`. The first thing in the way is **`CW_F3_Body_07`**, an F3 bodyshell panel whose face is at **y 5126**. It is a plain `StaticMeshActor` implementing nothing, so the sweep stops there and `ALTTrain` is never considered. The train's own interact-bearing actor `CW_Train` blocks Visibility only through `CarriageMesh` at **y 5200**, 74 uu behind the shell. Traced from the pawn's real eye point and control rotation: with nothing ignored the first hit is at reach **~205**; with `CW_F3_*` ignored it moves to reach **~275** and is `CW_Train`. Ignoring `CW_F3_Body_07` alone drops the hit count to **0**; ignoring `CW_Train` alone leaves it at **1**. That pair of results is what identifies the blocker, and an earlier cumulative-exclusion sweep that appeared to blame `CW_Train` was wrong for exactly that reason: exclusions had accumulated. <br> **A second, thinner problem.** Even with the shell ignored the train sits at **275 uu**, outside the 250 range from a comfortable standing spot. The capsule can close to about y 5076 before the shell stops it, which would put `CarriageMesh` 124 uu away and in range, so the shell is the primary fault, but there is little margin. **`BoardingVolume` cannot rescue this**: `ALTTrain` builds it `QueryOnly` with `ECR_Overlap` on every channel, and an overlap response does not block a sweep, so it is invisible to the trace. <br> **Not fixed, deliberately.** The brief scoped this session to verification and said a verified failure is a good outcome. The candidate fixes are listed in known-issues 2.11; the most general of them is a `Source/` change to sweep multi and take the first hit that implements the interface, and `Source/` was out of bounds here. **This needs a decision before C1/C3 can be called done.** <br> **Two tooling notes.** (1) **PIE in the level-editor viewport loses input focus**: `playtest_key` keeps reporting `consumed:true` while the pawn's velocity stays exactly zero, and `playtest_console("Teleport")` is accepted and does nothing. Two sessions were lost to this before the cause was clear. **`playtest_start({use_new_window=true})` fixes it** and input stayed reliable for the whole session. Worth reaching for first in any PIE round that needs to drive the player. (2) The ticket-barrier line at y 4125 has **90 uu gaps** on a 450 uu pitch, barriers being 360 wide; the walk to the platform has to thread one, at x 5700 for the lane in front of the player start. <br> **Acceptance.** 29 gameplay actor transforms hashed **1057580773 before and after**, unchanged, over the same 29-actor set F3c used (`Gameplay`, `Spawns`, `Anchors`, `Interchange`, `TrainShell` plus `CW_Train` and `CW_DepartureBoard`); the absolute figure differs from F3c's 3356498618 only because the fold string differs, and the actor list is identical. Three PIE sessions, **all stopped cleanly with `playtest_stop`, no EndPlayMap crash, and nothing mutated while PIE was running**. No asset, Blueprint, map or `Source/` file was edited, so there was no test scaffolding to revert: god mode was a runtime console command only. Docs touched: this row and known-issues 2.11. <br> **Also spotted, minor and unrelated.** The HUD points label renders an unlocalised text key, for example `Mac-5F869E425B10C8`, in place of a name. Recorded in 2.11. |
