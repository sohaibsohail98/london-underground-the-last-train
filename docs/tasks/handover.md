...
| S11 Station signage + surface retint, both maps | **COMMITTED `f635ece` 2026-09-08, but the S12 review (2026-09-09) found the signs are not actually readable in-game - treat S11 as "assets shipped, placement broken".** Saved to disk at 12:30 before an editor crash on a `MAP CHECK` console exec at 12:31; state reconciled and committed as its own step. The crash-resume reconciliation checked property values (text string, material paths, transforms present) but **never rendered a sign from player height**, which is how the two MAJOR defects below slipped through. **New assets (all fine):** `M_LT_SignCharcoal` + `M_LT_SignViolet` (charcoal back panel `#16161C`, violet name bar `#6C4C9C`, both palette-exact per S12) and `M_LT_Pictogram`; 14 `MI_Picto_*` under `Materials/Signage/Pictograms/` off the S9-haul ISO 7010 pictogram textures (S12 confirms no roundel / no Johnston / no operator livery, ISO 7010 is an international safety standard not operator IP). **Surface retint (fine):** walls to `MI_Tiles133A` (greybox) / `MI_Tiles107` (Canary Wharf), floor to `MI_Concrete048` / `MI_Concrete046`, the 6 greybox platform columns (`GBX_Column_1..6`) and both Canary Wharf tunnel mouths to `MI_Concrete031`; 10 `MI_<Set>` touched, all load clean. S12 swept every material assignment on both maps: **zero `WorldGridMaterial`, no checkerboard.** The 14 Canary Wharf actors on engine `BasicShapeMaterial` (2 train carriages, 4 escalator pieces, 8 gameplay anchor markers) are exactly the documented intentional list. **BROKEN, from S12 (fix these):** (1 MAJOR) all six station-name Text actors are `Pitch=90,Yaw=0` while their Back/Bar panels are `Yaw=-90` - the glyphs render edge-on (text normal `(0,0,-1)` vs panel `(0,-1,0)`) and read as a hairline; and the Text Y sits 2cm behind the panel on the wall side so it is occluded anyway. Fix: set the Text actors to `Pitch=0,Yaw=-90` and move them to the room side (GBX `Y=-742`, CW `Y=83`). (2 MAJOR) `TextRenderColor` on all six is `(238,235,235)` = `#EEEBEB` off-white, NOT the sodium `#E0A030` the commit and this row claimed. Fix: set to `(224,160,48)`. (3 MINOR) greybox has **6** pictogram actors, not the 9 claimed here and in the commit body - Canary Wharf's 14 are correct. (4 MINOR, S12) the 26 `T_Picto_*` textures imported in `5e3e609` are `TC_EditorIcon` (uncompressed RGBA8, ~27MB) - should be `TC_Default`. Full S12 report is under the "## S12 Opus fresh-eyes review" heading. Both maps still pass `MAP CHECKDEP` 0/0; the sign defects are placement/colour, not asset validity. |
