...
export class CameraRig {
  ...
  update(dt: number): void {
    ...
    const leanX = MathUtils.clamp(
      (this.aim.x - this.focus.x) * 0.35,
      -o.lookaheadMax,
      o.lookaheadMax,
    );
    const leanZ = MathUtils.clamp(
      (this.aim.z - this.focus.z) * 0.35,
      -o.lookaheadMax,
      o.lookaheadMax,
    );
    ...
    this.velocityMagnitude =
      this.camera.position.distanceTo(this.previousPosition) / Math.max(dt, 1e-4);
