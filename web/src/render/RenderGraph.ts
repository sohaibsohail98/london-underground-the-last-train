...
export class RenderGraph {
  ...
  get activeStages(): StageName[] {
    return (Object.keys(this.stages) as StageName[]).filter(
      (name) => this.stages[name] !== undefined,
    );
  }
  ...
  build(): void {
    ...
    if (this.toggles.motionBlur) {
      ...
      ldr = mix(source, stage.node, this.amounts.motionBlur);
    }
    ...
    if (this.toggles.chromatic) {
      ...
      ldr = mix(source, stage.node as unknown as Node<'vec4'>, this.amounts.chromatic);
    }
    ...
    if (this.toggles.grain) {
      ...
      ldr = mix(ldr, stage.node as unknown as Node<'vec4'>, this.amounts.grain);
    }
    ...
    if (this.toggles.vignette) {
      ...
      ldr = mix(ldr, stage.node, this.amounts.vignette);
    }
    ...
    if (this.toggles.sharpen) {
      ...
      ldr = mix(source, stage.node, this.amounts.sharpen);
    }
